...
const Beranda: React.FC = () => {
    ...
    if (userId) {
      try {
        const response = await fetch(
          ...
          }/api/main/get-link-project/${userId}`
        );
  ...
  const handleAddLink = async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_PREFIX_BACKEND}/api/main/add-link-project`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        Swal.fire("Success!", "Berhasil menambahkan project", "success");
      }

      // Fetch updated project links
      ...
      if (updatedData.success) {
        setProjectLinks(updatedData.data);
      }
  ...
  const handleEditLink = async () => {
    try {
      const response = await fetch(
        `${
          import.meta.env.VITE_PREFIX_BACKEND
        }/api/main/edit-link-project/${currentLinkId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      Swal.fire("Success!", "Berhasil mengedit project", "success");
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(
            `${
              import.meta.env.VITE_PREFIX_BACKEND
            }/api/main/delete-link-project/${id}`,
            {
              method: "DELETE",
            }
          );
          ...
          Swal.fire(
            "Deleted!",
            "Your project link has been deleted.",
            "success"
          );
  ...
  return (
    <div className="flex h-screen mb-6">
      ...
      <div className="flex-1 p-6 pt-20 md:ml-64 lg:ml-64 transition-all duration-300">
        <h2 className="text-2xl font-bold">Add Hyperlink Project</h2>
        <button
