...
const ViewOutput: React.FC = () => {
  ...
  const fetchPhotoBio = async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_PREFIX_BACKEND}/api/main/get-photo-bio/${idProject}`
      );
  ...
  const fetchmenuLinks = async () => {
    if (userId) {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_PREFIX_BACKEND}/api/main/get-link-menu/${idProject}`
        );
        if (!response.ok) {
      ...
      <div className="flex-1 p-6 pt-20 lg:ml-64 transition-all duration-300 flex flex-col items-center">
        ...
        <div
          className={`border rounded-lg p-4 shadow-md flex flex-col justify-between ${
            device === "mobile" ? "w-80 h-[500px]" : "w-full max-w-screen-lg h-[600px]"
          } overflow-y-scroll`}
        >
          {/* Photo and Bio */}
            ...
            {loading ? (
              ...
              <div className="flex flex-col gap-4">
                {menuLinks.map((link) => (
                  ...
                  >
                    <h3 className="text-lg font-semibold overflow-hidden">{link.title}</h3>
                  </div>
                ))}
              </div>
            ) : (
